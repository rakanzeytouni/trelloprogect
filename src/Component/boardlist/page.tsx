...
// ✅ Define Board type — assuming your DB returns `id` as number
type Board = {
  id: number;           // 👈 Changed from string to number (match your DB)
  title: string;
  creatAT: string;      // Consider renaming to createdAt for consistency
  updateAt: string;     // Consider renaming to updatedAt
};
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-yellow-200 to-orange-100 p-4 sm:p-6 md:p-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
        {boards.map((board) => {
          // ✅ Validate board.id is a valid number
          const boardIdNum = Number(board.id);
          if (isNaN(boardIdNum)) {
            console.warn("Skipping board with invalid ID:", board.id);
            return null;
          }
          ...
          return (
            ...
            >
              <div className="relative p-4 sm:p-6 flex flex-col justify-between h-40 sm:h-48">
                ...
                {/* ✅ Pass validated number ID */}
                <Deleteboard bordeId={boardIdNum} />
              </div>
            </Link>
          );
        })}
